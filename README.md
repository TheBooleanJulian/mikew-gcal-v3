...
**Auto-posts every Friday at 8 PM SGT** with next week's schedule, or trigger manually anytime.
...
|---|---|
| `/schedule` | Post **this week's** schedule (Mon–Sun) |
| `/nextweek` | Post **next week's** schedule |
| `/help` | Show help message |
| `/start` | Same as `/help` |

The bot also **auto-posts every Friday at 8 PM SGT**, showing the upcoming Mon–Sun schedule.
...
```
mikewinacbot/
├── bot.py            Telegram bot + APScheduler weekly cron
...
You should see:
```
2026-03-23 20:00:00 | INFO | __main__ | Scheduler started — weekly post every Friday 20:00 SGT
2026-03-23 20:00:00 | INFO | __main__ | Bot polling…
```

Test it by sending `/schedule` in the Telegram group.
...
4. **Confirm it's running**
   ...
   - Send `/schedule` in your Telegram group
...
**Auto-post not firing on Fridays**
- The schedule runs at 20:00 **SGT (UTC+8)**. If your server is in a different timezone this is handled automatically by pytz.
- Verify the bot is still running: `docker ps` or check Zeabur logs

**"No upcoming bookings found"**
- The NAC page only shows bookings that have been confirmed. If Kew hasn't booked slots yet for next week, the page will be empty for that period.
...
**Scraper returns wrong week**
- `/schedule` → current Mon–Sun
- `/nextweek` → following Mon–Sun
- The Friday auto-post always sends **next week** (since you want the schedule ready for the weekend ahead)
...
| `apscheduler` | 3.10.4 | Friday night cron scheduler |
